...
router.get("/get-favourites", async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ message: "Favourite Profiles Got Successfully", favourites });
  } catch (error) {
    console.log(error);
    res.status(500).json({ message: "Internal Server Error" });
  }
...
router.put("/message", async (req, res) => {
  try {
    ...
    if (!user.interested.includes(id))
      return res.status(400).json({ message: "Reciver Unmatched" });
    ...
    async function updateMessage({ sender, reciver }) {
      ...
      if (messageData[reciver]) {
        ...
        let message = [...messageData[reciver][day], data];
        messageData[reciver] = message;
      } else {
        //else create one for the reciver and add data
        let message = [data];
        messageData[reciver][day] = message;
      }
